...
Observing with `str()` shows me that I have a DF with 60 rows, one character variable and two numerica variables.

It also looks like the treatment variable is categorical. lets fix it.
...
This shows us that there are 3 complete cases in the first 6 rows.
...
Sometimes it would be nice to determine the number of rows with complete cases or the number of NAs in a data frame or vector.
...
df_remove_NA_mass_2 <- df[complete.cases(df$Dried_mass_g), ]
...
```{r}
df1 <- na.omit(df) # does the same thing as complete.cases(): removes all cases with an NA value
View(df1)
```
...
```{r}
rm(df1, df_remove_All, df_remove_NA_Mass, df_remove_NA_growth) # removes objects with these names
```
...
Our predictor variable type is: *Fertilizer Treatment which is a Categorical data*

Our response variable type is: *Plant growth rate which is a Ratio data*
...
```{r}
all_model <- lm(Growth_rate_mm ~ Treatment, data = df_All_NA)
```
...
*Two of the three treatments can vary.*

Why are there 46 DF for the Residuals?

*The number of NAs in growth is 11. 60-11 = 49. Then we subtract the number of groups, 49-3 = 46.*
